    ...
    try {
        ...
        const defaultSettings = [
            ...
            },
            {
                ...
                value: 'Dúvidas no cadastro? Fale conosco: +258 YY YYY YY',
...
const planSchema = new mongoose.Schema({
    ...
    investmentAmount: { type: Number, required: true, unique: true },
...
userRouter.post('/notifications/delete', async (req, res) => {
    ...
    try {
        ...
        if (modifiedCount === 0 && !deleteAll && notificationStatusIds && notificationStatusIds.length > 0) {
            const count = await UserNotificationStatus.countDocuments({ userId: userId, _id: { $in: notificationStatusIds }});
    ...
    try {
        ...
        if ((user.balance.MT || 0) < investmentAmount) {
            return res.status(400).json({ 
                message: `Saldo insuficiente (${(user.balance.MT || 0).toFixed(2)} MT) para ativar o plano "${planToActivate.name}" (${investmentAmount.toFixed(2)} MT). Por favor, realize um depósito.`,
...
adminRouter.patch('/withdrawals/:withdrawalId', async (req, res) => {
    ...
    try {
        ...
        if (status === 'Rejeitado') {
            ...
            if (user) {
                user.balance.MT = (user.balance.MT || 0) + withdrawal.amount; // Devolve o valor total ao saldo principal
                await user.save();
