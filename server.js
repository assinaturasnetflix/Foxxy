...
// -----------------------------------------------------------------------------
// 3. CONEXÃO COM O MONGODB E CRIAÇÃO DE ADMIN INICIAL
// -----------------------------------------------------------------------------
...
async function createInitialAdmin() {
    ...
    try {
        const existingAdmin = await mongoose.model('User').findOne({ email: adminEmail.toLowerCase() }); // Usar mongoose.model aqui pois User pode não estar definido ainda
        ...
        const securityAnswer = "admin_recovery_code_123"; // Será hasheada no pre-save

        // A definição do User precisa estar disponível. O hash será feito pelo hook pre-save do UserSchema.
        ...
        await newAdmin.save(); // Isso vai disparar o pre-save hook para hashear senha e securityAnswer
        ...
        // Se o modelo User ainda não foi definido quando esta função é chamada,
        // podemos ter um erro aqui. A chamada createInitialAdmin() precisa ser feita
        // depois que os modelos são definidos.
        if (error.message.includes("Schema hasn't been registered for model \"User\"")) {
             ...
             // Adiar a execução ou garantir que os modelos sejam carregados antes.
             // Por simplicidade, vamos garantir que createInitialAdmin é chamado após os modelos.
        } else {
...
// A conexão e chamada createInitialAdmin() serão movidas para DEPOIS da definição dos Schemas.
// Isso garante que o modelo 'User' esteja disponível.
// server.js
...
// -----------------------------------------------------------------------------
// ... (final da seção de SCHEMAS da Parte 2) ...
...
.then(async () => { // Adicionado async aqui
    ...
    await createInitialAdmin(); // Chamar após a conexão E após os modelos serem definidos
    await getSiteSettings(); // Inicializar o cache de configurações do site
})
...
async function getSiteSettings() {
    if (siteSettingsCache) {
        // Em um cenário de produção, adicionar uma lógica de expiração de cache aqui seria ideal
        // Ex: if (siteSettingsCache && siteSettingsCache.timestamp > Date.now() - CACHE_DURATION) return siteSettingsCache.data;
    ...
    try {
        ...
        siteSettingsCache = formattedSettings; // Poderia ser { data: formattedSettings, timestamp: Date.now() }
        // console.log("Configurações do site carregadas/recarregadas no cache.");
...
// Inicializa o cache ao iniciar, após a conexão com o DB e definição dos modelos.
// A chamada será feita após a conexão com o MongoDB.
...
publicRouter.use('/blog', blogPublicRouter); // Monta o mini-router para o blog público
...
userRouter.post('/claims', async (req, res) => {
    ...
    const siteConfig = await getSiteSettings();
    const allowedClaimCurrencies = siteConfig.allowedClaimCurrencies || ["MT", "BTC", "ETH", "USDT"]; // ["MT", "BTC", "ETH", "USDT"];
...
// 10.3.2. Criar Plano (POST /api/admin/plans) - Já atualizado na Parte 5 do desenvolvimento incremental
// A versão consolidada reflete a atualização:
...
// 10.3.3. Editar Plano (PUT /api/admin/plans/:planId) - Já atualizado na Parte 5 do desenvolvimento incremental
// A versão consolidada reflete a atualização:
...
// 10.10. Gerenciamento de Promoções Urgentes (Admin) - Definido na Parte 1 do desenvolvimento incremental
// (POST /api/admin/urgent-promotions)
...
// (GET /api/admin/urgent-promotions)
...
// (PUT /api/admin/urgent-promotions/:promotionId)
...
// (DELETE /api/admin/urgent-promotions/:promotionId)
adminRouter.delete('/urgent-promotions/:promotionId', async (req, res) => {
    ...
    try {
        ...
        // Opcional: deletar imagem associada se estiver armazenada localmente
...
// 10.11. Gerenciamento de Banners da Homepage (Admin) - Definido na Parte 3 do desenvolvimento incremental
...
// (PUT /api/admin/homepage-banners/:bannerId)
...
// (DELETE /api/admin/homepage-banners/:bannerId)
...
// 10.12.1. Categorias do Blog (Admin) - Definido na Parte 4 do desenvolvimento incremental
...
blogCategoryRouter.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 100, search = '' } = req.query; // Aumentado limite padrão para categorias
        const query = {};
        if (search) {
            query.$or = [
                { name: { $regex: search, $options: 'i' } },
...
// 10.12.2. Tags do Blog (Admin) - Definido na Parte 4 do desenvolvimento incremental
...
blogTagRouter.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 100, search = '' } = req.query; // Aumentado limite padrão para tags
        const query = {};
        if (search) {
            query.$or = [
                { name: { $regex: search, $options: 'i' } },
...
// 10.12.3. Posts do Blog (Admin) - Definido na Parte 4 do desenvolvimento incremental
...
blogPostRouter.put('/:postId', async (req, res) => {
    ...
    try {
        ...
        if (!currentPost) return res.status(404).json({ message: "Post não encontrado para atualização." });
        if (status === 'published' && currentPost.status !== 'published') {
            if (!publishedAt || new Date(publishedAt) <= new Date()) { finalPublishedAt = new Date(); }
        }
        const updatedPostData = { title, slug: postSlug, content, excerpt, coverImage, category: category || null, tags: tags || [], status, isFeatured, publishedAt: finalPublishedAt, seoTitle, seoDescription, seoKeywords, updatedAt: Date.now() };
        const updatedPost = await BlogPost.findByIdAndUpdate(postId, updatedPostData, { new: true, runValidators: true }).populate('category', 'name slug').populate('tags', 'name slug').populate('author', 'name email');
        if (!updatedPost) return res.status(404).json({ message: "Post não encontrado." });
        res.json(updatedPost);
    } catch (error) {
        if (error.code === 11000) return res.status(409).json({ message: "Outro post com este slug já existe (erro de duplicidade)." });
...
// Montar o adminRouter principal na aplicação
// Esta linha será movida para o final do arquivo, após todas as definições de rotas.
// app.use('/api/admin', adminRouter);
// server.js
// ... (continuação da Parte 9) ...

// (adminRouter continua e é finalizado aqui)
app.use('/api/admin', adminRouter); // Monta o adminRouter principal na aplicação
